...
-- concat 연산자를 사용하면 여러문장인 경우에는 간단하게 작성 가능
SELECT 'Oracle ' || '21C ' || 'Good..!!' from dual;
-- 위 문장을 concat() 함수를 이용한 SQL문으로 변경하면 =>
...
from employees;
--2단계 : 성과 이름을 연결했지만 띄어쓰기가 없어 가독성이 떨어짐
select concat(first_name, last_name), salary, department_id
from employees;
--3단계 : 성과 이름을 연결하고 띄어쓰기를 연산자로 붙여 공백 생성
--4단계 : 컬럼명이 너무 길게 출력되므로 별칭(full_name)을 부여한다.
...
형식] trim([leading | trailing | both] 제거할 문자 from 컬럼)
    - leading: 좌측
    - trailing: 우측
    - both: 양쪽에서 제거(default)
    [주의1] 양쪽 끝의 문자만 제거되고 중간의 문자는 제거되지 않음
    [주의2] '문자'만 제거할 수 있고 '문자열'은 제거할 수 없다.
*/
select
    trim('다' from '다람쥐가 나무를 탑니다') --양쪽의 '다' 제거
    , trim(both '다' from '다람쥐가 나무를 탑니다') --both는 default
from dual;
select
    trim(leading '다' from '다람쥐가 나무를 탑니다') -- 좌측 '다' 제거
    , trim(trailing '다' from '다람쥐가 나무를 탑니다') -- 우측 '다' 제거
    , trim(' 다람쥐가 나무를 탑니다 ') -- 양쪽 공백 제거
from dual;
...
select substr('good morning john', 9) from dual; -- ning john
select substr('good morning john', -8, 4) from dual; -- ing

--문자열의 길이를 반환한다.
select length('good morning') from dual;

/*
시나리오] 사원테이블의 frist_name을 첫글자를 제외한 나머지 부분을
    *로 마스킹 처리하는 쿼리문을 작성하시오.
*/
-- 1단계: 이름의 첫글자만 출력하기(인덱스 1부터 1글자를 잘라낸다.)
select first_name, substr(first_name, 1, 1) from employees;
-- 2단계: 이름을 10글자로 간주하여 나머지 부분을 *로 채워보기
select rpad(first_name, 10, '*') from employees;
/* 3단계: 이름의 첫글자를 갖겨오고 이름의 길이(length)를 얻어온다.
...
replace(): 문자열을 다른 문자열로 대체할때 사용한다. 만약 공백으로
    ...
    대체할 수 있으므로 trim()에 비해 replace()가 훨씬 사용빈도가 높다.
*/
-- 문자열을 변경한다.
...
from dual;
-- trim()과 같이 공백을 제거한다. 단 문자열 중간의 공백도 제거한다.
