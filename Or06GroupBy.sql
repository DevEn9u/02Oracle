...
group by: 동일한 값이 있는 레코드를 하나의 그룹으로 묶어서 인출한다.
    보여지는건 하나의 레코드지만 다수의 레코드가 하나의 그룹으로 묶어진 결과이므로
    통계적인 값을 계산할 수 있다.
...
/*
count() 함수를 통해 인출된 행의 개수는 아래와 같이 일반적인 SELECT 문으로
검증할 수 있다.
*/
...
-- 예제] 10번 부서에 근무하는 사람들의 급여 합계는 얼마인지 출력하시오.
select
...
/*
    ...
    *을 사용하는 것을 권장한다. 컬럼의 특성 혹은 데이터에 따른
    방해를 받지 않으므로 실행속도가 더 빠르다.
*/

/*
count() 함수의 사용법
1) count(all 컬럼명)
    => default 사용법으로 컬럼 전체의 레코드를 기준으로 카운트한다.
...
select
    ...
    count(distinct job_id) "순수전체개수"
from employees;

/*
avg(): 평균값을 구할때 사용하는 함수
*/
-- 예제] 전체사원의 평균급여는 얼마인지 출력하는 쿼리문을 작성하시오.
select
    count(*) "전체사원수",
    sum(salary) "급여합계",
    sum(salary) / count(*) "평균급여(직접계산)",    
    avg(salary) "평균급여(avg함수이용)",
    ltrim(to_char(avg(salary), '999,000.00')) "평균급여(정리까지함)"
from employees;

--예제] 영업팁(SALES)의 평균급여는 얼마인가요?
/*
1. 부서테이블에서 영업팀의 부서번호가 몇번인지 확인한다.
하지만 데이터의 대소문자가 다르므로 결과가 인출되지 않는다.
*/
...
-- initcap()으로 첫글자만 대문자로 만들어도 된다.
select * from departments where department_name = initcap('SALES');
/*
3. 80번 부서(Sales)에서 근무하는 사원들의 평균급여를 구해 출력한다.
*/
select 
...
/*
min() / max() : 최댓값, 최솟값을 찾을때 사용하는 함수
*/
-- 예제] 전체 사원중 급여가 가장 적은 직원은 누군가요?
/*
...
-- 예제] 전체 사원중 가장 적은 급여는 얼마인가요? 즉 급여의 최솟값을 얼마인가요?
...
-- 위 2개의 쿼리문을 합치면 아래와 같은 서브쿼리문이 된다.
...
기술하면 에러가 발생한다. 각 레코드별로 서로 다른 이름이 저장되어 있으므로
그룹의 조건에 단일 컬럼을 사용할 수 없기 때문이다.
*/
select department_id, sum(salary), first_name
from employees group by department_id; -- first_name에 의해 에러발생
...
select
    department_id,
    rtrim(to_char(sum(salary), '999,000')) "sum_salary",
    count(employee_id)"cnt_employee",
    rtrim(to_char(avg(salary),'999,000'))"avg_salary"
...
*/
/*
같은 부서에서 근무하더라도 담당업무는 다를 수 잇으므로 이 문제에서는
group by절에 2개의 컬럼을 명시해야 한다. 즉 부서로 그룹화 한 후 
다시 담당업무별로 그룹화한다.
*/
-- 잘못된 쿼리문, where절 사용 불가
select 
...
/*
담당업무별 사원수는 물리적으로 존재하는 컬럼이 아니므로 where절에 추가하면
에러가 발생한다. 이 경우에는 having절에 조건을 추가해야 한다.
ex) 급여가 3000인 사원 => 물리적으로 존재하므로 where절에 추가
    평균급여가 3000인 사원 => 개발자가 상황에 맞게 논리적으로 만들어낸
                           결과이므로 having절에 추가해야 함
*/
-- 앞에서 발생한 문제는 where절을 having절로 조건을 변경하면 해결됨.
select 
    department_id, job_id, count(*), avg(salary)
from employees
group by department_id, job_id
...
-- 최저급여 3000 미만의 조건은 그룹함수를 통해 만들어진 결과이므로 having절에 기술
...
*/
select
    rtrim(to_char(max(salary), '999,000')) MaxPay, min(salary) MinPay, 
...
select
    job_id, count(*) "직원합계"
from employees group by job_id order by 직원합계;
...
*/
select
    department_id, count(*), rtrim(to_char(avg(salary),'999,000.00')) "평균급여"
